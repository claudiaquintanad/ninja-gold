...
def processmoney(request):
    if 'contador' in request.session:
        if request.POST['ninja'] == 'Farm': 
            ...
            request.session['salida'].append({'text': f"Earned {earn} golds from the casino!", 'color': 'verde', 'date': strftime("%H:%M %p", localtime())})
        ...
        if request.POST['ninja'] == 'Casino': 
            ...
            if earn > 0:
                ...
                request.session['salida'].append({'text': f"Entered a casino and lost {earn} golds...Ouch...", 'color': 'rojo', 'date': strftime("%H:%M %p", localtime())})
                request.session.save()
